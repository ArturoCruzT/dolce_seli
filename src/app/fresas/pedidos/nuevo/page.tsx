...
// CAMBIOS:
//  • Sin navbar móvil inferior (eliminado completamente)
//  • Layout PC: catálogo izquierda + carrito/checkout derecha (side-by-side)
//  • Formulario cliente: grid 2 columnas en desktop, 1 columna en móvil
//  • Paso "cliente" en desktop: panel izquierdo resumen + derecho formulario
//  • Sigue funcionando perfecto en móvil
// ============================================================
    ...
    <div className="bg-gray-50 rounded-2xl p-4 mb-3">
      ...
      <div className="flex flex-wrap gap-2">
        {disponibles.map(t => {
          ...
          return (
            <ToppingChip key={t.id} t={t} sel={sel}
...
// ============================================================
// RESUMEN CARRITO (reutilizable para sidebar y paso cliente)
// ============================================================

const ResumenCarrito = ({ carrito, subtotal, envio, total, envioGratis, costoEnvio, compact = false }: {
  carrito: ItemCarrito[]; subtotal: number; envio: number; total: number;
  envioGratis: boolean; costoEnvio: number; compact?: boolean;
}) => (
  <div className={`bg-white rounded-dolce-lg shadow-dolce ${compact ? 'p-3' : 'p-4'}`}>
...
export default function POSPage() {
  ...
  const agregarIndividual = (prod: ProductoIndividual) => {
    setItemEnConfig({
      id: uid(), tipo: 'individual', productoId: prod.id, nombre: prod.nombre,
  ...
  const agregarPaquete = (paq: Paquete) => {
    const componentes: ComponentePaquete[] = paq.productosIncluidos.map(pi => {
      ...
      return {
        productoId: pi.productoId, nombre: prod?.nombre || 'Producto',
        emoji: prod?.emoji, toppingsIncluidos: prod?.toppingsIncluidos || 0, toppings: [],
      };
    });
    setItemEnConfig({
      id: uid(), tipo: 'paquete', productoId: paq.id, nombre: paq.nombre,
  ...
  const confirmarPedido = async () => {
    ...
    const items: ItemPedido[] = carrito.map(item => ({
      ...
      toppingsSeleccionados: item.toppings.map(t => ({ toppingId: t.id, toppingNombre: t.nombre })),
      toppingsPorItem: item.componentesPaquete?.map((c, idx) => ({
        itemIndex: idx,
        toppings: c.toppings.map(t => ({ toppingId: t.id, toppingNombre: t.nombre })),
      })),
    }));
  ...
  if (paso === 'toppings' && itemEnConfig) return (
    <div className="min-h-screen bg-cream-dolce flex flex-col">
      ...
      {/* Contenido — en desktop centrado y más ancho */}
      <div className="flex-1 overflow-y-auto p-4 pb-32 max-w-2xl mx-auto w-full">
        {itemEnConfig.tipo === 'individual' && (
          <PanelToppings
            titulo={itemEnConfig.nombre} emoji={itemEnConfig.emoji}
            toppingsIncluidos={itemEnConfig.toppingsIncluidos}
            seleccionados={itemEnConfig.toppings} disponibles={toppings}
  ...
  // ── PASO CLIENTE ─────────────────────────────────────────
  // Layout: móvil = columna, desktop = dos columnas (izq formulario, der resumen)
        ...
        <div className="max-w-6xl mx-auto px-4 py-5 lg:grid lg:grid-cols-[1fr_380px] lg:gap-6 lg:items-start">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-dolce-lg shadow-dolce p-5">
              ...
              {/* 2 columnas en desktop para campos de cliente */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input type="text" value={nombre} onChange={e => setNombre(e.target.value)}
                  placeholder="Nombre *"
                  className="w-full px-4 py-3 border border-gray-300 rounded-dolce focus:ring-2 focus:ring-pink-deep focus:border-transparent text-base transition-all" />
                <input type="tel" value={telefono} onChange={e => setTelefono(e.target.value)}
                  placeholder="Teléfono *"
                  className="w-full px-4 py-3 border border-gray-300 rounded-dolce focus:ring-2 focus:ring-pink-deep focus:border-transparent text-base transition-all" />
                <input type="text" value={direccion} onChange={e => setDireccion(e.target.value)}
                  placeholder="Dirección de entrega"
                  className="w-full px-4 py-3 border border-gray-300 rounded-dolce focus:ring-2 focus:ring-pink-deep focus:border-transparent text-base transition-all sm:col-span-2" />
                <input type="url" value={linkMaps} onChange={e => setLinkMaps(e.target.value)}
                  placeholder="Link Google Maps (opcional)"
                  className="w-full px-4 py-3 border border-gray-300 rounded-dolce focus:ring-2 focus:ring-pink-deep focus:border-transparent text-base transition-all sm:col-span-2" />
              </div>
            </div>

            {/* Sección Entrega + Envío en grid de 2 para desktop */}
              ...
              <div className="bg-white rounded-dolce-lg shadow-dolce p-5">
                ...
                {!envioGratis && (
                  <div className="relative">
                    ...
                    <input type="number" value={costoEnvio}
            ...
            <div className="bg-white rounded-dolce-lg shadow-dolce p-5">
              ...
              <textarea value={notas} onChange={e => setNotas(e.target.value)}
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-dolce focus:ring-2 focus:ring-pink-deep text-base resize-none"
                rows={3} />
            </div>

            {/* Botón confirmar — visible en móvil aquí, en desktop solo en sidebar */}
          ...
          {/* ── COLUMNA DERECHA: resumen (solo desktop, sticky) ── */}
  ...
  // ── CATÁLOGO PRINCIPAL ───────────────────────────────────
  // Desktop: catálogo izquierda + carrito sidebar derecha
  return (
    <div className="min-h-screen bg-cream-dolce">
      ...
      <div className="bg-white shadow-sm px-4 py-3 sticky top-0 z-20">
        <div className="flex items-center justify-between max-w-6xl mx-auto">
          ...
          {/* Total visible en móvil en el header */}
          {hayItems && (
      ...
      {/* Layout: móvil = stack, desktop = catálogo + sidebar */}
      <div className="max-w-6xl mx-auto lg:grid lg:grid-cols-[1fr_360px] lg:gap-6 lg:items-start px-4 py-5"
        ...
        <div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {paq.productosIncluidos.map((pi, i) => {
                            const prod = individuales.find(p => p.id === pi.productoId);
                            return (
                              <span key={i} className="text-xs bg-pink-50 text-pink-deep px-2 py-0.5 rounded-full font-medium">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-xs text-gray-400">
                          {paq.toppingsIncluidos} topping{paq.toppingsIncluidos !== 1 ? 's' : ''} incl.
                        </p>
                      </div>
                    </div>
                  </button>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              {individuales.map(prod => (
                <button key={prod.id} type="button" onClick={() => agregarIndividual(prod)}
                  ...
                  <p className="text-xs text-gray-400">
                    {prod.toppingsIncluidos} topping{prod.toppingsIncluidos !== 1 ? 's' : ''} incl.
                  </p>
                </button>
              ))}
            </div>
          ...
          {/* CARRITO — solo en móvil (en desktop va en sidebar) */}
          {hayItems && (
                    ...
                    <div key={item.id} className="bg-white rounded-dolce-lg shadow-dolce p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {item.toppings.map((t, i) => (
                                <span key={i} className={`text-xs px-2 py-0.5 rounded-full ${
                                  i < item.toppingsIncluidos ? 'bg-green-100 text-green-700' : 'bg-pink-100 text-pink-700'
                                }`}>
                          ...
                          {extra > 0 && <p className="text-xs text-pink-deep mt-1 font-medium">+${extra} extra</p>}
                        </div>
        ...
        {/* ── COLUMNA DERECHA: sidebar carrito (solo desktop) ── */}
                      ...
                      <div key={item.id} className="flex items-start gap-3 py-2 border-b border-gray-100 last:border-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.tipo === 'individual' && item.toppings.map((t, i) => (
                            <span key={i} className={`inline-block text-xs px-1.5 py-0.5 rounded-full mr-1 mt-0.5 ${
                              i < item.toppingsIncluidos ? 'bg-green-100 text-green-700' : 'bg-pink-100 text-pink-700'
                            }`}>
                              {t.nombre}
                            </span>
                        ...
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                          ...
                          <div className="flex items-center gap-1">
                            <button onClick={() => cambiarCantidad(item.id, -1)}
                              className="w-6 h-6 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs font-bold text-gray-700">−</button>
      ...
      {/* Checkout fijo — solo en móvil */}
      {hayItems && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t-2 border-gray-100 px-4 py-4"
          style={{ boxShadow: '0 -4px 20px rgba(0,0,0,0.10)' }}>
          <button onClick={() => setPaso('cliente')}
